...
class App extends React.Component {
  ...
  asyncLoadComponents = () => {
    import(/* webpackChunkName: "common"*/ "../routes/common").then(
      (CommonComponents) => {
        this.setState({ CommonComponents });
      }
    );
  };
  ...
  render() {
    const { topRoutes, mainRoutes, CommonComponents } = this.state;
    return (
      <div style={{ display: "flex" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ height: "80px", borderBottom: "3px solid pink" }}>
            Top panel
            {renderRoutes(topRoutes, { CommonComponents })}
          </div>
          {/* Main panel */}
          <div>
            Main panel
            {renderRoutes(mainRoutes, { CommonComponents })}
          </div>
        </div>
      </div>
    );
